...
import com.kevolution.repository.MemberAddressRepository;
...
public class MypageService {
    ...
    // 배송지
    public List<MemberAddress> getAddresses(String userId) {
        return memberAddressRepository.findByMember(getMember(userId));
    }
    ...
    public void addAddress(String userId, String recipient, String phone,
        ...
        Member member = getMember(userId);
        if (isDefault) {
            memberAddressRepository.findByMemberAndDefaultAddressTrue(member)
                    .ifPresent(a -> a.setAsDefault(false));
        }
        memberAddressRepository.save(MemberAddress.builder()
                .member(member)
                .recipient(recipient)
                .phone(phone)
                .zipcode(zipcode)
